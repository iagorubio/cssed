                           ...
                           CRAdditionalSel * a_sel, xmlNode * a_node)
{
        ...
        for (; node; node = get_next_parent_element_node (node)) {
                ...
                if (val
                    && !strncmp ((const gchar*)val,
                                 (const gchar*)a_sel->content.pseudo->extra->stryng->str,
...
class_add_sel_matches_node (CRAdditionalSel * a_add_sel, xmlNode * a_node)
{
        ...
        if (xmlHasProp (a_node, (const xmlChar*)"class")) {
                ...
                for (cur = klass; cur && *cur; cur++) {
                        ...
                        if (!strncmp ((const gchar*)cur, 
                                      (const gchar*)a_add_sel->content.class_name->stryng->str,
...
id_add_sel_matches_node (CRAdditionalSel * a_add_sel, xmlNode * a_node)
{
        ...
        if (xmlHasProp (a_node, (const xmlChar*)"id")) {
                ...
                if (!strncmp ((const gchar*)id, (const gchar*)a_add_sel->content.id_name->stryng->str,
                ...
                case EQUALS:
                        {
                                ...
                                if (value
                                    && strcmp 
                                    ((const gchar*)value, 
                                     (const gchar*)cur_sel->value->stryng->str)) {
                                        xmlFree (value);
                                        return FALSE;
                                }
                                xmlFree (value);
        ...
        for (cur_node = a_node; cur_sel; cur_sel = cur_sel->prev) {
                if (((cur_sel->type_mask & TYPE_SELECTOR)
                     && (cur_sel->name 
                         && cur_sel->name->stryng
                         && cur_sel->name->stryng->str)
                     && (!strcmp ((const gchar*)cur_sel->name->stryng->str,
                                  (const gchar*)cur_node->name)))
                                                ...
                                                enum CRPseudoType a_type)
{
        ...
        for (elem = PRIVATE (a_this)->pcs_handlers;
             elem; elem = g_list_next (elem)) {
                entry = elem->data;
                if (!strcmp ((const gchar*)entry->name, (const gchar*)a_name)
                    && entry->type == a_type) {
                        found = TRUE;
                        break;
                }
                                              ...
                                              a_handler)
{
        ...
        for (elem = PRIVATE (a_this)->pcs_handlers;
             elem; elem = g_list_next (elem)) {
                entry = elem->data;
                if (!strcmp ((const gchar*)a_name, (const gchar*)entry->name)
                    && entry->type == a_type) {
                        found = TRUE;
                        break;
                }
