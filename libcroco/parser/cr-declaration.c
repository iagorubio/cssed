                               ...
                               const guchar * a_str, enum CREncoding a_enc)
{
        ...
        parser = cr_parser_new_from_buf ((guchar*)a_str, strlen ((const gchar*)a_str), a_enc, FALSE);
...
cr_declaration_to_string (CRDeclaration * a_this, gulong a_indent)
{
	...
	if (a_this->property 
		...
		str = g_strndup ((const gchar*)a_this->property->stryng->str,
                                 ...
                                 const guchar * a_prop)
{
		...
		if (cur->property 
			...
			if (!strcmp ((const gchar*)cur->property->stryng->str, (const gchar*)a_prop)) {
				return cur;
			}
		}
	}
